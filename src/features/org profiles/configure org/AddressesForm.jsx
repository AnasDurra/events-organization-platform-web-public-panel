import { Button, Col, Divider, Empty, Form, Row, Select, Skeleton, Typography } from 'antd';
...
export default function AddressesForm({ org, isGetOrgLoading, form, addOrgAddress, removeOrgAddress, loading }) {
    const { data: { result: addresses } = { result: [] }, isLoading: isAddressesLoading } = useGetAddressesQuery();
                    ...
                    >
                        ...
                        >
                            <Select
                                options={addresses
                                    ?.filter(
                                        (addr) => !org?.addresses?.some((orgAddr) => orgAddr.address?.id == addr.id)
                                    )
                                    ?.map((addr) => ({
                                        value: addr.id,
